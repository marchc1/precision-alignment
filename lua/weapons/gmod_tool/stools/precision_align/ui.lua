...
function STACK_POPUP:Init()
	...
	self.slider_stackamount.Text.OnEnter = function() self.button_ok:DoClick() end
...
function STACK_POPUP:Paint(w, h)
	if self.m_bBackgroundBlur then Derma_DrawBackgroundBlur(self, self.m_fCreateTime) end
...
function INDICATOR:Paint(w, h)
	...
	if PA_funcs.construct_exists(textbox:GetListView().construct_type, textbox:GetID()) then draw.RoundedBox(6, 0, 0, self:GetWide(), self:GetTall(), Color(0, 230, 0, 255)) end
end
...
local function TextFocusOn(pnl)
	if pnl:GetClassName() == "TextEntry" and pnl.Type == "PA" then PA_manipulation_panel:SetKeyBoardInputEnabled(true) end
end

hook.Add("OnTextEntryGetFocus", "PAKeyboardFocusOn", TextFocusOn)
local function TextFocusOff(pnl)
	if pnl:GetClassName() == "TextEntry" and pnl.Type == "PA" then PA_manipulation_panel:SetKeyBoardInputEnabled(false) end
end
...
function FUNCTION_BUTTON_2:OnMousePressed(mousecode)
	if not self.Depressed then
		...
		for k, v in pairs(function_buttons_2_list) do
			if v.Depressed then v.Depressed = false end
		end
...
function CONSTRUCT_MULTISELECT:Init()
	...
	self.list_points.DoDoubleClick = function(Line, LineID) self.list_points:ClearSelection() end
	...
	self.list_lines.DoDoubleClick = function(Line, LineID) self.list_lines:ClearSelection() end
	...
	self.list_planes.DoDoubleClick = function(Line, LineID) self.list_planes:ClearSelection() end
	...
	self.button_attach:SetFunction(function()
		for k, v in pairs(self.list_points:GetSelected()) do
			ID = v:GetID()
			if PA_funcs.construct_exists("Point", ID) then PA_funcs.attach_point(ID, PA_activeent) end
		end

		for k, v in pairs(self.list_lines:GetSelected()) do
			ID = v:GetID()
			if PA_funcs.construct_exists("Line", ID) then PA_funcs.attach_line(ID, PA_activeent) end
		end

		for k, v in pairs(self.list_planes:GetSelected()) do
			ID = v:GetID()
			if PA_funcs.construct_exists("Plane", ID) then PA_funcs.attach_plane(ID, PA_activeent) end
		end
		return true
	end)
...
function CONSTRUCT_MULTISELECT:SelectAll(value)
	local function SelectLines(value, panel, construct_table)
		for id = 1, 9 do
			...
			if self.visibility then construct_table[id].visible = value end
		end
	end
...
function CONSTRUCT_MULTISELECT:GetSelection()
	...
	for k, v in pairs(self.list_points:GetSelected()) do
		local ID = v:GetID()
		if PA_funcs.construct_exists("Point", ID) then table.insert(selection.points, ID) end
	end

	for k, v in pairs(self.list_lines:GetSelected()) do
		local ID = v:GetID()
		if PA_funcs.construct_exists("Line", ID) then table.insert(selection.lines, ID) end
	end

	for k, v in pairs(self.list_planes:GetSelected()) do
		local ID = v:GetID()
		if PA_funcs.construct_exists("Plane", ID) then table.insert(selection.planes, ID) end
	end

	if (#selection.points + #selection.lines + #selection.planes) == 0 then selection = nil end
	return selection
end
...
local function Open_Manipulation_Tab(Tab)
	...
	if not PA_manipulation_panel:IsVisible() then PA_manipulation_panel:SetVisible(true) end
end
...
function TOOL_POINT_PANEL:Init()
	...
	self.list_primarypoint.OnRowSelected = function(panel, line) PA_selected_point = line end
	...
	self.button_delete:SetFunction(function() return PA_funcs.delete_point(PA_selected_point) end)
	self.button_attach:SetFunction(function() return PA_funcs.attach_point(PA_selected_point, PA_activeent) end)
	self.button_deleteall:SetFunction(function()
		self.list_primarypoint:SelectFirstItem()
		self.list_secondarypoint:SelectFirstItem()
...
function TOOL_LINE_PANEL:Init()
	...
	self.list_line.OnRowSelected = function(panel, line) PA_selected_line = line end
	...
	self.button_delete:SetFunction(function() return PA_funcs.delete_line(PA_selected_line) end)
	self.button_attach:SetFunction(function() return PA_funcs.attach_line(PA_selected_line, PA_activeent) end)
	self.button_deleteall:SetFunction(function()
		self.list_line:SelectFirstItem()
...
function TOOL_PLANE_PANEL:Init()
	...
	self.list_plane.OnRowSelected = function(panel, line) PA_selected_plane = line end
	...
	self.button_delete:SetFunction(function() return PA_funcs.delete_plane(PA_selected_plane) end)
	self.button_attach:SetFunction(function() return PA_funcs.attach_plane(PA_selected_plane, PA_activeent) end)
	self.button_deleteall:SetFunction(function()
		self.list_plane:SelectFirstItem()
...
function TOOL_LIST:Init()
	...
	self.list_tooltype.OnRowSelected = function(parent, line, isselected) RunConsoleCommand(PA_ .. "tooltype", tostring(line)) end
...
local function umsg_click_hook()
	...
	-- Points
	if tooltype <= 4 then
		if shift then select_next_point() end
	...
	elseif tooltype == 5 then
		if shift then select_next_line() end
		...
		if alt then
			...
			if PA_funcs.construct_exists("Line", PA_selected_line) and precision_align_lines[PA_selected_line].entity ~= ent then PA_funcs.attach_line(PA_selected_line, ent) end
		end
	...
	elseif tooltype == 8 then
		if shift then select_next_plane() end
...
cvars.AddChangeCallback(PA_ .. "size_point", function(CVar, Prev, New)
	point_size_min = tonumber(math.max(New, 1))
	point_size_max = tonumber(New) * 1000
end)

cvars.AddChangeCallback(PA_ .. "size_line_start", function(CVar, Prev, New) line_size_start = tonumber(New) end)
cvars.AddChangeCallback(PA_ .. "size_line_end", function(CVar, Prev, New)
	line_size_min = tonumber(New)
	line_size_max = line_size_min * 1000
end)
...
local function inview(pos2D)
	if pos2D.x > -ScrW() and pos2D.y > -ScrH() and pos2D.x < ScrW() * 2 and pos2D.y < ScrH() * 2 then return true end
	return false
end
		...
		if v.visible and v.startpoint and v.endpoint then
			...
			if line_temp then
				...
				if inview(line_end) then
					...
					surface.DrawLine(line_end.x - norm_dir_2D.x * size2, line_end.y - norm_dir_2D.y * size2, line_end.x + norm_dir_2D.x * size2, line_end.y + norm_dir_2D.y * size2)
					surface.DrawLine(line_end.x + (line_dir_2D.x / 3 - norm_dir_2D.x) * size2, line_end.y + (line_dir_2D.y / 3 - norm_dir_2D.y) * size2, line_end.x + (line_dir_2D.x / 3 + norm_dir_2D.x) * size2, line_end.y + (line_dir_2D.y / 3 + norm_dir_2D.y) * size2)
				end
