...
PA_funcs.move_entity = function(vec1, vec2, activeent)
	...
	RunConsoleCommand(PA_ .. "move", tostring(v.x), tostring(v.y), tostring(v.z), tostring(stack))
	return true
end
...
PA_funcs.rotate_entity = function(ang, vec, relative, activeent)
	...
	Message(msgstring)
	RunConsoleCommand(PA_ .. "rotate", tostring(ang.p), tostring(ang.y), tostring(ang.r), tostring(vec.x), tostring(vec.y), tostring(vec.z), tostring(relative), tostring(stack))
	return true
end
...
PA_funcs.construct_exists = function(construct_type, ID)
	...
	if construct_type == "Point" then
		if precision_align_points[ID].origin then return true end
	elseif construct_type == "Line" then
		if precision_align_lines[ID].startpoint and precision_align_lines[ID].endpoint then return true end
	elseif construct_type == "Plane" then
		if precision_align_planes[ID].origin and precision_align_planes[ID].normal then return true end
	end
	return false
end
...
PA_funcs.point_global = function(point)
	...
	if not point_temp.origin then return false end
...
PA_funcs.line_global = function(line)
	...
	if not line_temp.startpoint or not line_temp.endpoint then return false end
...
PA_funcs.set_line = function(line, startpoint, endpoint, direction, length)
	...
	if direction then
		...
		if ent then direction = ent:WorldToLocal(ent:GetPos() + direction):GetNormal() end
...
PA_funcs.plane_global = function(plane)
	...
	if not plane_temp.origin or not plane_temp.normal then return false end
...
PA_funcs.plane_function_perpendicular = function(planeID1, planeID2)
	if PA_funcs.construct_exists("Plane", planeID1) and PA_funcs.construct_exists("Plane", planeID2) then
		...
		if vec:Length() == 0 then Warning("Tried to find cross product of two parallel planes") end
		return vec
	end
...
local function matrix_x_vector(matrix, v)
	...
	local v2 = matrix[2]
	local v3 = matrix[3]
	return Vector(v1.x * v.x + v1.y * v.y + v1.z * v.z, v2.x * v.x + v2.y * v.y + v2.z * v.z, v3.x * v.x + v3.y * v.y + v3.z * v.z)
end
...
local function matrix_transpose(matrix)
	...
	local v2 = matrix[2]
	local v3 = matrix[3]
	return {Vector(v1.x, v2.x, v3.x), Vector(v1.y, v2.y, v3.y), Vector(v1.z, v2.z, v3.z)}
end

-- Rotate angle by world angles
PA_funcs.rotate_world = function(ang, rotang)
	if rotang.p ~= 0 then ang:RotateAroundAxis(Vector(0, 1, 0), rotang.p) end
	if rotang.y ~= 0 then ang:RotateAroundAxis(Vector(0, 0, 1), rotang.y) end
	if rotang.r ~= 0 then ang:RotateAroundAxis(Vector(1, 0, 0), rotang.r) end
	return ang
end
...
PA_funcs.rotate_2lines_parallel = function(pivot, lineID1, lineID2, activeent)
	local line1 = PA_funcs.line_global(lineID1)
	local line2 = PA_funcs.line_global(lineID2)
	if not pivot then pivot = line1.startpoint end
	...
	if dir1 == Vector(-dir2.x, -dir2.y, -dir2.z) then ang = Angle(-ang.p, ang.y + 180, -ang.r) end
...
PA_funcs.rotate_2planes_parallel = function(pivot, planeID1, planeID2, activeent)
	local plane1 = PA_funcs.plane_global(planeID1)
	local plane2 = PA_funcs.plane_global(planeID2)
	if not pivot then pivot = plane1.origin end
...
PA_funcs.rotate_line_plane_parallel = function(pivot, axis, lineID, planeID, activeent)
	...
	local M = {-axisdir:Cross(normal):Cross(normal):GetNormal(), axisdir:Cross(normal):GetNormal(), normal}
	...
	local function solver(x, y)
		...
		if Handedness < 0 then Ang = -Ang end
		return Ang
	end
	...
	if not pivot and axis then pivot = axis.startpoint end
	return PA_funcs.rotate_entity(ang, pivot, 0, activeent)
end
...
PA_funcs.plane_mirror_entity = function(planeID, activeent)
	...
	local shift = LocalPlayer():KeyDown(IN_SPEED)
	local stack = 0
	if shift then stack = GetConVarNumber(PA_ .. "stack_num") end
	-- Mirror by concommand directly
	RunConsoleCommand(PA_ .. "mirror", tostring(origin.x), tostring(origin.y), tostring(origin.z), tostring(normal.x), tostring(normal.y), tostring(normal.z), tostring(stack))
