...
local function Message(text)
	if GetConVarNumber(PA_ .. "display_messages") == 1 then LocalPlayer():ChatPrint("(PA) " .. text) end
end

local function Warning(text)
	if GetConVarNumber(PA_ .. "display_warnings") == 1 then LocalPlayer():ChatPrint("(PA) ERROR: " .. text) end
end
...
function MANIPULATION_FRAME:Init()
	...
	self.button_help:SetFunction(function() return gui.OpenURL("https://sourceforge.net/userapps/mediawiki/wenli/index.php?title=Precision_Alignment") end)
	-- Alpha slider
...
function DISPLAYS_TAB:Init()
	...
	self.panel_multiselect.list_points.OnRowSelected = function(Panel, Line) update_visibility(Panel, precision_align_points) end
	self.panel_multiselect.list_lines.OnRowSelected = function(Panel, Line) update_visibility(Panel, precision_align_lines) end
	self.panel_multiselect.list_planes.OnRowSelected = function(Panel, Line) update_visibility(Panel, precision_align_planes) end
	...
	-- Initialize colour settings
	self.colourcircle_enthighlight:SetColor(GetConVarNumber(PA_ .. "selectcolour_h"), GetConVarNumber(PA_ .. "selectcolour_s"), GetConVarNumber(PA_ .. "selectcolour_v"), GetConVarNumber(PA_ .. "selectcolour_a"))
	self.colourcircle_attachment:SetColor(GetConVarNumber(PA_ .. "attachcolour_h"), GetConVarNumber(PA_ .. "attachcolour_s"), GetConVarNumber(PA_ .. "attachcolour_v"), GetConVarNumber(PA_ .. "attachcolour_a"))
end
...
function POINTS_TAB:Init()
	...
	self.checkbox_relative1.OnChange = function() if not update_primary_listview() and self.checkbox_relative1:GetChecked() then self.checkbox_relative1:SetValue(false) end end
	...
	self.button_set:SetFunction(function()
		...
		if selection then
			...
			if self.checkbox_relative1:GetChecked() and IsValid(PA_activeent) then vec = PA_activeent:LocalToWorld(vec) end
	...
	self.button_delete:SetFunction(function()
		local selection = self.list_primarypoint:GetSelectedLine()
		if selection then return PA_funcs.delete_point(selection) end
		Warning("No selection")
		return false
	end)
	...
	self.button_deleteall:SetFunction(function() return PA_funcs.delete_points() end)
	...
	self.checkbox_relative2.OnChange = function() if not update_secondary_listview() and self.checkbox_relative2:GetChecked() then self.checkbox_relative2:SetValue(false) end end
	...
	self.functions_list_functionpoints.OnRowSelected = function(LineID, Line) end
...
function LINES_TAB:Init()
	...
	self.button_delete:SetFunction(function()
		local selection = self.list_primary:GetSelectedLine()
		if selection then return PA_funcs.delete_line(selection) end
		Warning("No selection")
		return false
	end)
	...
	self.button_deleteall:SetSize(80, 25)
	self.button_deleteall:SetText("Delete All")
	self.button_deleteall:SetFunction(function() return PA_funcs.delete_lines() end)
	...
	self.checkbox_relative.OnChange = function() if not update_primary_listview() and self.checkbox_relative:GetChecked() then self.checkbox_relative:SetValue(false) end end
	...
	self.button_update_startpoint:SetFunction(function() return update_sliders_point() end)
	...
	self.button_update_endpoint:SetFunction(function() return update_sliders_point() end)
	...
	self.button_update_direction:SetFunction(function() return update_sliders_direction() end)
	...
	self.button_update_length:SetFunction(function() return update_sliders_length() end)
	...
	self.functions_list_functionlines.OnRowSelected = function(panel, line) end
...
function PLANES_TAB:Init()
	...
	self.button_delete:SetFunction(function()
		local selection = self.list_primary:GetSelectedLine()
		if selection then return PA_funcs.delete_plane(selection) end
		Warning("No selection")
		return false
	end)
	...
	self.button_deleteall:SetSize(80, 25)
	self.button_deleteall:SetText("Delete All")
	self.button_deleteall:SetFunction(function() return PA_funcs.delete_planes() end)
	self.checkbox_relative = vgui.Create("DCheckBoxLabel", self)
	self.checkbox_relative:SetPos(140, 30)
	self.checkbox_relative:SetText("Relative to\nEntity")
	self.checkbox_relative:SetSize(100, 30)
	...
	self.button_update_normal:SetFunction(function() return update_sliders_normal() end)
	...
	self.functions_list_functionplanes.OnRowSelected = function(panel, line) end
...
function MOVE_TAB:Init()
	...
	self.button_axisrotate_angle:SetFunction(function()
		...
		if pivot_selection then if PA_funcs.construct_exists("Point", pivot_selection) then pivot = PA_funcs.point_global(pivot_selection).origin end end
	...
	self.button_move_constructs:SetFunction(function()
		...
		elseif selection_type == "Line" then
			...
			if not line then
				Warning("Select Line 1")
				return false
			elseif not PA_funcs.construct_exists("Line", line) then
				Warning("Line " .. tostring(line) .. " not defined")
				return false
			end
		...
		elseif selection_type == "Plane" then
			...
			if not plane then
				Warning("Select Plane 1")
				return false
			elseif not PA_funcs.construct_exists("Plane", plane) then
				Warning("Plane " .. tostring(plane) .. " not defined")
				return false
			end
	...
	self.list_pivotpoint.DoDoubleClick = function(Line, LineID) self.list_pivotpoint:ClearSelection() end
...
function FUNCTIONS_TAB:Init()
		...
		for k, v in pairs(selection_lists) do
			if v:IsVisible() then
				...
				if #selection ~= numselections then
					...
					if selection_type[k + 1] > 1 then str = str .. "s" end
					Warning(str)
					return false
				end
	...
	self.button_point_threeplanes.func = function(selection_primary, selection_secondary)
		...
		if origin then return PA_funcs.set_point(selection_primary, origin) end
		return false
	end
	...
	self.button_line_2planes.func = function(selection_primary, selection_secondary)
		...
		if line then return PA_funcs.set_line(selection_primary, line.startpoint, nil, line.direction) end
		return false
	end
	...
	self.button_line_plane_projection.func = function(selection_primary, selection_secondary)
		...
		if line then return PA_funcs.set_line(selection_primary, line.startpoint, line.endpoint) end
		return false
	end
	...
	self.button_plane_3points.func = function(selection_primary, selection_secondary)
		...
		if plane_temp then return PA_funcs.set_plane(selection_primary, plane_temp.origin, plane_temp.direction) end
		return false
	end
...
function ROTATION_TAB:Init()
	...
	self.button_rotate:SetFunction(function()
		...
		local vec
		if pivot then if PA_funcs.construct_exists("Point", pivot) then vec = PA_funcs.point_global(pivot).origin end end
		local relative = 0
		if self.checkbox_relative1:GetChecked() then
			relative = 1
...
function ROTATION_FUNCTIONS_TAB:Init()
	...
	self.button_set:SetFunction(function()
		...
		if not self.activebutton.func(pivot, axis, selections) then return false end
	end)
	...
	self.list_line_axis:SetMultiSelect(false)
	self.list_line_axis.DoDoubleClick = function(Line, LineID) self.list_line_axis:ClearSelection() end
	-- FUNCTION BUTTONS ************************************************************************************************
	...
	self.button_angle_2lines.func = function(pivot, axis, selections) return PA_funcs.rotate_2lines_parallel(pivot, selections[1], selections[2], PA_activeent) end
	...
	self.button_angle_2planes.func = function(pivot, axis, selections) return PA_funcs.rotate_2planes_parallel(pivot, selections[1], selections[2], PA_activeent) end
	...
	self.button_lineplane_normal.func = function(pivot, axis, selections) return PA_funcs.rotate_lines_planenormal_parallel(pivot, selections[1], selections[2], PA_activeent) end
	...
	self.button_lineplane.func = function(pivot, axis, selections) return PA_funcs.rotate_line_plane_parallel(pivot, axis, selections[1], selections[2], PA_activeent) end
	...
	self.button_mirror.func = function(pivot, axis, selections) return PA_funcs.plane_mirror_entity(selections[1], PA_activeent) end
end
...
function CONSTRAINTS_TAB:Init()
	...
	self.list_point_LPos2.DoDoubleClick = function(Line, LineID) self.list_point_LPos2:ClearSelection() end
	...
	self.button_constraint:SetFunction(function()
		...
		if not PA_funcs.construct_exists("Point", selection2) then
			point2 = {
				["origin"] = PA_funcs.point_global(selection1).origin + Vector(0, 0, 1) -- Set so default axis dir is (0,0,1)
			}
		else
...
function CONSTRAINTS_AXIS_TAB:Init()
	...
	self.list_point_axis.DoDoubleClick = function(Line, LineID) self.list_point_axis:ClearSelection() end
...
function CONSTRAINTS_BALLSOCKET_TAB:Init()
	...
	self.Constraint_Func = function() return 0 end
end
