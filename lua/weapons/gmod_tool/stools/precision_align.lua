...
if CLIENT then
	...
	for i = 1, 9 do
		precision_align_points[i] = {
			visible = true
		}

		precision_align_lines[i] = {
			visible = true
		}

		precision_align_planes[i] = {
			visible = true
		}
	end
	...
	function TOOL:SetActive(ent)
		...
		local function Deselect(ent)
			if IsValid(ent) and ent.PA then
				...
				if colour then ent:SetColor(colour) end
		...
		-- Deselect last ent
		if activeent then Deselect(activeent) end
		-- Select new ent
		if IsValid(ent) then
			-- Check for existing player selection
			if ent.PA then Deselect(ent) end
...
function TOOL:GetClickPosition(trace)
	...
	if not IsValid(Phys) then return trace.HitPos end
	...
	elseif Edge_Snap or Centre_Snap then
		...
		if Centre_Snap then
			...
			if (math.abs(Edge_Dist.x) < math.abs(Centre_Dist.x) and Snapped_Edges.x) or not Snapped_Centres.x then NewPosL.x = EdgePosL.x end
			if (math.abs(Edge_Dist.y) < math.abs(Centre_Dist.y) and Snapped_Edges.y) or not Snapped_Centres.y then NewPosL.y = EdgePosL.y end
			if (math.abs(Edge_Dist.z) < math.abs(Centre_Dist.z) and Snapped_Edges.z) or not Snapped_Centres.z then NewPosL.z = EdgePosL.z end
		end
...
function TOOL:RightClick(trace)
	...
	if alt then precision_align_lastaction_func(ply) end
	return true
end
