...
local function playsound(ply, bool)
	if bool then ply:EmitSound("buttons/button14.wav", 100, 100) end
end
...
local function Queue_Process()
	...
	if ent_table.nocollide then
		...
		if IsValid(last_ent) then constraint.NoCollide(ent, last_ent) end
...
local function Stack_Undo(undo_table) -- if table.Count( undo_table ) == 0 then return end
	...
	local model = ""
	if IsValid(undo_table[1].ent) then model = undo_table[1].ent:GetModel() or "" end
...
function precision_align_move_func(ply, cmd, args)
	...
	-- Begin stacking
	if stack > 0 then stack_ID = stack_ID + 1 end
	for stacks = 1, math.max(stack, 1) do
		-- Add to stack queue
		if stack > 0 then Queue_Add(ply, ent, stack_ID) end
		-- Move entity
...
function precision_align_rotate_func(ply, cmd, args)
	...
	if not IsValid(ent) then return false end
	if not util.IsValidPhysicsObject(ent, 0) or IsValid(ent:GetParent()) then return false end
	if not tonumber(args[1]) or not tonumber(args[2]) or not tonumber(args[3]) then return false end
	local startpos = ent:GetPos()
	local startang = ent:GetAngles()
	...
	if stack >= 2 then a = ent:WorldToLocalAngles(a) end
	-- Begin stacking
	for stacks = 1, math.max(stack, 1) do
		-- Add to stack queue
		if stack > 0 then Queue_Add(ply, ent) end
		-- Rotate entity
		local pos = ent:GetPos()
		local ang = ent:GetAngles()
		local localv
...
function precision_align_constraint_func(len, ply)
	...
	if CPPI then
		if Ent1:IsWorld() then
			if not util.IsValidPhysicsObject(Ent2, 0) or not Ent2:CPPICanTool(ply, PA) then return false end
		elseif Ent2:IsWorld() then
			if not util.IsValidPhysicsObject(Ent1, 0) or not Ent1:CPPICanTool(ply, PA) then return false end
	...
	if constraint_type == "Axis" then
		...
		local axis
		if vars and vars ~= 0 then axis = Ent1:WorldToLocal(Ent1:LocalToWorld(LPos1) + Vector(vars.x, vars.y, vars.z)) end
	...
	if const then
		...
		undo.SetPlayer(ply)
		undo.Finish()
		if Ent1 then Ent1:GetPhysicsObject():EnableMotion(false) end
		if Ent2 then Ent2:GetPhysicsObject():EnableMotion(false) end
		playsound(ply, true)
	end
	return const
end
